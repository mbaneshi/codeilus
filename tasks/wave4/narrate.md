...
- `crates/codeilus-narrate/Cargo.toml` — Updated: added codeilus-graph, codeilus-parse, codeilus-db, tokio, serde deps + petgraph dev-dep
- `crates/codeilus-narrate/src/lib.rs` — Created: module declarations, `generate_all_narratives()` public API
- `crates/codeilus-narrate/src/types.rs` — Created: Narrative struct, `narrative_kind_key()` helper
- `crates/codeilus-narrate/src/prompts.rs` — Created: PromptTemplate struct, `get_prompt()` with all 8 narrative kinds
- `crates/codeilus-narrate/src/placeholders.rs` — Created: data-driven placeholder content for all 8 narrative kinds
- `crates/codeilus-narrate/src/generator.rs` — Created: NarrativeGenerator with `generate_all()`, `generate_one()`, `explain_symbol()`
- `crates/codeilus-narrate/tests/narrate.rs` — Created: 9 test cases
- `crates/codeilus-db/src/repos/narrative_repo.rs` — Created: NarrativeRepo (insert, insert_batch, get_by_kind, get_by_kind_and_target, list, list_by_kind, delete_all)
- `crates/codeilus-db/src/repos/mod.rs` — Updated: added narrative_repo module and re-exports
- `crates/codeilus-db/src/lib.rs` — Updated: added NarrativeRepo, NarrativeRow re-exports
- `crates/codeilus-db/tests/repos.rs` — Updated: added 3 narrative_repo test cases

### Tests:
```
codeilus-narrate: 9 passed, 0 failed
  prompt_template_overview, prompt_template_all_kinds, prompt_template_has_context_placeholder,
  placeholder_overview, placeholder_reading_order, placeholder_module_summary,
  generator_placeholder_mode, generator_all_kinds_covered, narrative_content_not_empty

codeilus-db narrative_repo: 3 passed
  narrative_repo_insert_and_get, narrative_repo_get_by_kind_and_target, narrative_repo_list_by_kind
```

### Clippy:
Zero warnings for codeilus-narrate.

### Issues / Blockers:
- The `narratives` DB table schema (id, kind, target_id, language, content, generated_at) does not have `title` or `is_placeholder` columns. The NarrativeRepo matches the actual schema. Title and is_placeholder live only in the in-memory `Narrative` struct.
- codeilus-llm dependency was skipped since it's being built in parallel and only has stubs. The NarrativeGenerator currently runs in placeholder mode only.

### Notes:
- **Placeholder mode** is always active since codeilus-llm is not wired yet. To enable LLM mode, the `NarrativeGenerator` needs a `placeholder_mode: false` path that builds context from the graph and calls the LLM with prompt templates from `prompts.rs`.
- **NarrativeRepo** maps to the actual `narratives` table schema. The `language` column defaults to `'en'` and `generated_at` is auto-set by SQLite.
- **ReadingOrder** and **ModuleSummary** placeholders are data-driven (use entry point scores and community members from the graph), so they provide useful content even without an LLM.
- **SymbolExplanation** is on-demand only — not pre-generated by `generate_all()`.
- The `narrative_kind_key()` function converts NarrativeKind to DB string keys for storage.
- All 8 prompt templates include `{context}` placeholder and follow PocketFlow patterns (beginner-friendly analogies, concise language).
